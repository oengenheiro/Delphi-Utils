...
// This unit is a full copy of the Vcl.Styles; it only patches the TStyleEngine.DoUnregisterStyleHook @ Line 3456
// Fix by Alex Kononov 2013 01 04 - Prevents Access Violation upon application shutdown
// See http://qc.embarcadero.com/wc/qcmain.aspx/qcmain.aspx?d=123278
...
begin
  ...
  // Alex Kononov 2013 01 04
  // Temporary fix to prevent access violation
  //  if (LItem.Key is ControlClass) and (LItem.Value is StyleHookClass) then
    if (LItem.Value is StyleHookClass) and (LItem.Key is ControlClass) then
    begin
  ...
  case TThemedSearchIndicators(State) of
    tsiTextNormal, tsiTextHot, tsiTextPressed, tsiTextDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnText', GetState(State - Integer(tsiTextNormal)));

    tsiAudioNormal, tsiAudioHot, tsiAudioPressed, tsiAudioDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnAudio', GetState(State - Integer(tsiAudioNormal)));
  end;
end;
...
begin
  case TThemedToggleSwitch(State) of
    ttsTrackOffNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOffElement, ssNormal);
    ttsTrackOffHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOffElement, ssHot);
    ttsTrackOffDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOffElement, ssDisabled);

    ttsTrackOnNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOnElement, ssNormal);
    ttsTrackOnHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOnElement, ssHot);
    ttsTrackOnDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CTrackOnElement, ssDisabled);

    ttsThumbNormal:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CThumbElement, ssNormal);
    ttsThumbHot:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CThumbElement, ssHot);
    ttsThumbDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect, CControlName, CThumbElement, ssDisabled);
  else
    Result := inherited DrawElement(Style, DC, Part, State, R, ClipRect);
  end;
end;
...
begin
  ...
  case TThemedMPlayerButtons(State) of
    tmpbPlayNormal, tmpbPlayHot, tmpbPlayPressed, tmpbPlayDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnPlay', GetState(State - Integer(tmpbPlayNormal)));
    tmpbPauseNormal, tmpbPauseHot, tmpbPausePressed, tmpbPauseDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnPause', GetState(State - Integer(tmpbPauseNormal)));
    tmpbStopNormal, tmpbStopHot, tmpbStopPressed, tmpbStopDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnStop', GetState(State - Integer(tmpbStopNormal)));
    tmpbNextNormal, tmpbNextHot, tmpbNextPressed, tmpbNextDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnNext', GetState(State - Integer(tmpbNextNormal)));
    tmpbPrevNormal, tmpbPrevHot, tmpbPrevPressed, tmpbPrevDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnPrev', GetState(State - Integer(tmpbPrevNormal)));
    tmpbStepNormal, tmpbStepHot, tmpbStepPressed, tmpbStepDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnStep', GetState(State - Integer(tmpbStepNormal)));
    tmpbBackNormal, tmpbBackHot, tmpbBackPressed, tmpbBackDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnBack', GetState(State - Integer(tmpbBackNormal)));
    tmpbRecordNormal, tmpbRecordHot, tmpbRecordPressed, tmpbRecordDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnRecord', GetState(State - Integer(tmpbRecordNormal)));
    tmpbEjectNormal, tmpbEjectHot, tmpbEjectPressed, tmpbEjectDisabled:
      Result := DrawButtonElement(TCustomStyle(Style), DC, R, ClipRect,
        ControlName, 'btnEject', GetState(State - Integer(tmpbEjectNormal)));
  end;
end;
